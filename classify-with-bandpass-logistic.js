...
// this will filter bandpass the data and consider LDA as dimensionality reduction method then classify the data near zero as unknown and filter it out
...
let coeffs = firCalculator.bandpass({
  order: filterOrder,
  Fs: sampleRate,
  F1: lowFreq,
  F2: highFreq,
...
const computeFeatures = (cspParams, eeg) => {
  ...
  let features = bci.windowApply(
    ...
    (trial) => {
      ...
      channels = channels.map((signal) =>
        filter.simulate(signal).slice(filterOrder)
      );
...
function logisticLearn(setA, setB) {
  ...
  let trainingSet = [].concat(
    ...[setA, setB].map((features, index) => {
